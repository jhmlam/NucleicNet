...
label2class = {0: 'NONSITE', 1: 'purine',
	2: 'pyrimidine', 3: 'RPHO', 4: 'RRIB'}
...
if __name__ == '__main__':
	...
	# load single grid for test
	# df = load_single_rna_grid('5jc3')
	...
	# df = pd.read_pickle('../blind_test/20180404_input/0000_Grid.pkl')
	df = pd.read_pickle(arg.input)

	# grid_label_raw = df['label'].values
	
	# be careful about the index
	grid_feature = df.iloc[:, 1:].values

	# the deep learning part
	# grid_label = label_one_hot(grid_label_raw)

	prob_test, test_pred_label, deep_feature = deep_learning_model(5,
		grid_feature,model_name)
	test_pred_label = np.array(test_pred_label)
	coarse_label = [label2class[x] for x in test_pred_label.tolist()]
	with open(arg.output+'_5_class_score.pickle','wb') as f:
		#cPickle.dump(prob_test, f)
                pickle.dump(prob_test, f)
	df['coarse_label'] = coarse_label #pd.Series(coarse_label)
	pickle.dump(df, open('%s_result.pickle' %(arg.output),'wb'))
